...
public class Cabeza_izquierda_01 : MonoBehaviour {
	...
	{
		if (GameManager.getTurno (0) == true && GameManager.isLife((int)Codigo_dragon.Dragon_1_1) && D2D_ReplaceOnCollision.disparo_curso == 1) {
			LetPlayer.turno = 1;
			CameraMov.turno = 1;
			...
			if (Input.GetMouseButton (0) == false && down == true) 
			{
				...
				if (angle2 < 50 && angle2 > -50) 
				{
					...
					{			
						...
						if (cloneRb2D != null) 
						{
							Vector2 velocidad = (endPos - startPos) * Power;
							cloneRb2D.velocity = velocidad;
			...
			if (Indicator != null && angle2 < 50 && angle2 > -50) 
			{
				...
				if (Camera.main != null && down == true) 
				{
					...
					var startPos = SpawnPoint.transform.position;
