...
public class Cabeza_derecha_01 : MonoBehaviour {
	...
	{
		if (GameManager.getTurno (3) == true && GameManager.isLife((int)Codigo_dragon.Dragon_1_2) && D2D_ReplaceOnCollision.disparo_curso == 1) 
		{
			LetPlayer.turno = 4;
			CameraMov.turno = 4;
			...
			{							
				...
				if (Indicator != null) 
				{
					...
					if (Camera.main != null && down == true) 
					{
						...
						Vector3 rotationVector2 = new Vector3 (0, rotacion, rotacion-angle2);
						Indicator.transform.localRotation = Quaternion.Euler (rotationVector2);
						if(scale/15 > 1)
						{
							scale = 15;
						}
			...
			if (Input.GetMouseButton (0) == false && down == true) 
			{
				...
				if (((angle2 > 130 && angle2 < 180)|| (angle2 < -130 && angle2 > -180))) 
				{
					if (Camera.main != null && Bullet != null) 
					{
						...
						xSp.x = xSp.x - 0.5f;
						var clone = D2D_Helper.CloneGameObject (Bullet, null, xSp, Quaternion.Euler (0, rotacion, rotacion-angle2));
